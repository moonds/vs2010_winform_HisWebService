 ...
 * 项目名称：银医V3.0
 * 作    者:
 * 文件名称：HisCallCustomInterfaceManager.cs
 * 描    述：His 接口调用接口管理类
/************************************************************************/
using System;
...
namespace InspurSelfService.BankHospitalFramework.Adapter
{
    ...
    /// 自定义接口管理器
    /// </summary>
    public class HisCallCustomInterfaceManager : BasePage
    {
        /// <summary>
        /// HisCallTrade
        /// </summary>
        ...
        /// <returns></returns>
        public static object HisCallTrade(string adapterTrace, out string resultCodeMsg, log4net.ILog log, string terminalIp, System.Reflection.MethodBase method, Dictionary<string, string> systemParams, Dictionary<string, string> customParams)
        {
            resultCodeMsg = null;
            object ret = null;
            ...
            try
            {
                ...
                ret = XMLHisResult(adapterTrace, inXml, outXml, out resultCodeMsg, log, resultCodeMsg);
        ...
        /// <param name="tradeCodeMsg"></param>
        /// <returns></returns>
        private static object HisCallResult(string adapterTrace, object retVal, out string resultCodeMsg, log4net.ILog log, string tradeCodeMsg)
        {
        ...
        private static Dictionary<string, object> AnalyzeOutXmlToDictionary(string outXml, string root)
        {
            ...
            if (!String.IsNullOrEmpty(outXml))
            {
                ...
                {
                    if (!values.ContainsKey(nodes.Item(i).Name))
                    {
